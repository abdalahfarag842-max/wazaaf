...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .navbar-brand-custom .brand-icon { background: #1a56db; color: #fff; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 16px; }
        ...
        .notif-badge { position: absolute; top: -2px; right: -4px; background: #1a56db; color: #fff; font-size: 10px; width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; }
        .user-menu { display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 4px 8px; border-radius: 8px; transition: background 0.15s; }
        .user-menu:hover { background: #f5f7fb; }
        .user-avatar { width: 34px; height: 34px; border-radius: 50%; background: #d1d5db; display: flex; align-items: center; justify-content: center; }
        ...
        .btn-mark-all { background: #f0f2f8; color: #1a56db; border: none; padding: 8px 16px; border-radius: 8px; font-size: 13px; font-weight: 500; cursor: pointer; transition: background 0.15s; }
        .btn-mark-all:hover { background: #dbeafe; }
        .notif-card { background: #fff; border: 1px solid #e5e9f0; border-radius: 12px; padding: 18px 20px; margin-bottom: 10px; display: flex; align-items: flex-start; gap: 14px; transition: all 0.15s; }
        .notif-card.unread { border-left: 4px solid #1a56db; background: #f8faff; }
        .notif-card:hover { box-shadow: 0 2px 12px rgba(26,86,219,0.07); }
        .notif-icon { width: 42px; height: 42px; border-radius: 10px; background: #dbeafe; color: #1a56db; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
        ...
        .btn-view { background: #1a56db; color: #fff; border: none; padding: 6px 14px; border-radius: 7px; font-size: 12px; font-weight: 500; text-decoration: none; transition: background 0.15s; }
        .btn-view:hover { background: #1447b8; color: #fff; }
        .btn-read { background: transparent; color: #9ca3af; border: 1px solid #e5e9f0; padding: 6px 10px; border-radius: 7px; font-size: 12px; cursor: pointer; transition: all 0.15s; }
        .btn-read:hover { border-color: #1a56db; color: #1a56db; }
        .empty-state { text-align: center; padding: 80px 20px; color: #9ca3af; background: #fff; border-radius: 12px; border: 1px solid #e5e9f0; }
...
<nav class="navbar-custom">
    ...
    <div class="nav-links">
        ...
        <a href="{{ route('candidate.jobs.index') }}" class="{{ request()->routeIs('candidate.jobs.*') ? 'active' : '' }}">Jobs</a>
        <a href="{{ route('candidate.applications.index') }}" class="{{ request()->routeIs('candidate.applications.*') ? 'active' : '' }}">My Applications</a>
        <a href="{{ route('candidate.notifications.index') }}" class="{{ request()->routeIs('candidate.notifications.*') ? 'active' : '' }}">Notifications</a>
    </div>
...
<div class="page-wrapper">
    <div class="page-header">
        ...
        @if(auth()->user()->unreadNotifications->count() > 0)
            <form action="{{ route('candidate.notifications.readAll') }}" method="POST">
                @csrf
            ...
            <div class="notif-card {{ is_null($notif->read_at) ? 'unread' : '' }}">
                ...
                <div class="notif-actions">
                    @if(isset($data['job_id']))
                        <a href="{{ route('candidate.jobs.show', $data['job_id']) }}" class="btn-view">View Job</a>
                    @endif
                    @if(is_null($notif->read_at))
                        <form action="{{ route('candidate.notifications.read', $notif->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn-read"><i class="fas fa-check"></i></button>
                        </form>
                    @endif
                </div>
            </div>

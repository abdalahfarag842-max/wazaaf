...
<section class="hero-section">
    <div class="hero-inner">
        <div class="hero-content">
            ...
            <form action="{{ route('candidate.home') }}" method="GET">
            ...
            <div class="popular-searches">
                ...
                @foreach($categories->take(5) as $cat)
                    <a href="{{ route('candidate.home', ['category' => $cat->id]) }}" class="popular-tag">{{ $cat->name }}</a>
                @endforeach
            </div>
        </div>
...
<div class="section-wrapper">
    <div class="section-header">
        ...
        <a href="{{ route('candidate.jobs.index') }}" class="section-link">View all →</a>
    </div>
    ...
    <div class="categories-grid">
        @foreach($categories->take(6) as $i => $cat)
            ...
            <a href="{{ route('candidate.home', ['category' => $cat->id]) }}" class="cat-card">
...
<div class="section-wrapper" style="padding-bottom: 40px;">
    ...
    <form action="{{ route('candidate.home') }}" method="GET">
        ...
        <div class="filters-bar">
            ...
            @if(request()->hasAny(['category','job_type','status']))
                <a href="{{ route('candidate.home', array_filter(['search'=>request('search'),'location'=>request('location')])) }}" class="filter-reset">
    ...
    @if($jobs->count())
        <div class="jobs-grid">
            @foreach($jobs as $i => $job)
                ...
                @endphp
                <a href="{{ route('candidate.jobs.show', $job) }}" class="job-card {{ $isClosed ? 'job-closed' : '' }}">
    ...
    @else
        <div class="empty-state">
            ...
            <a href="{{ route('candidate.home') }}" style="color:#1a56db;font-size:14px;margin-top:8px;display:inline-block;">Clear search</a>
        </div>
    @endif

<x-mainlayout>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <p class="stat-num" style="color: var(--color-success)">{{ $stats['open'] }}</p>
            <p class="stat-hint">Active now</p>
        </div>
        <div class="stat-card">
            <p class="stat-label">Closed</p>
            <p class="stat-num" style="color: var(--color-danger)">{{ $stats['closed'] }}</p>
            <p class="stat-hint">Ended</p>
        </div>
        <div class="stat-card">
            <p class="stat-label">Draft</p>
            <p class="stat-num" style="color: var(--color-warning)">{{ $stats['draft'] }}</p>
    ...
    <form method="GET" action="{{ route('jobs.index') }}" class="filter-bar">
        ...
        @if(request()->hasAny(['search', 'category_id', 'company_id', 'status']))
    ...
    <div class="table-card">
        ...
        @if($jobs->isEmpty())
            <div class="empty-state">
                ...
                <p>No jobs found. <a href="{{ route('jobs.create') }}">Create the first one.</a></p>
            </div>
        @else
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($jobs as $index => $job)
                            <tr>
                                <td class="text-muted">{{ $jobs->firstItem() + $index }}</td>
                                ...
                                <td>
                                    <i class="ti ti-map-pin"
                                        style="font-size:13px;vertical-align:-1px;color:#94A3B8"></i>
                                    {{ $job->location }}
                                </td>
                                ...
                                <td>
                                    <span class="apps-chip">
                                        {{ $job->applications_count ?? $job->applications->count() }} apps
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('jobs.show', $job) }}" class="btn-view">View</a>
                                        <a href="{{ route('jobs.edit', $job) }}" class="btn-edit">Edit</a>
                                        <form action="{{ route('jobs.destroy', $job) }}" method="POST" class="delete-form" style="display:contents">
                                            @csrf
                                            @method('DELETE')
